...
-(void)layoutItemView{
    ...
    //先判断当前的区域是否有cell
    BOOL topLeftFlag = NO;
    BOOL topRightFlag = NO;
    BOOL bottomLeftFlag = NO;
    BOOL bottomRightFlag = NO;
    
    for (WDHorizontalTableViewCell *cell in [self subviews]) {
        if ([cell isKindOfClass:[WDHorizontalTableViewCell class]]) {
            if (CGRectContainsPoint(currentRect, CGPointMake(cell.left, cell.top))) {
                topLeftFlag = YES;
            }
            if (CGRectContainsPoint(currentRect, CGPointMake(cell.right, cell.top))) {
                topRightFlag = YES;
            }
            if (CGRectContainsPoint(currentRect, CGPointMake(cell.left, cell.bottom))) {
                bottomLeftFlag = YES;
            }
            if (CGRectContainsPoint(currentRect, CGPointMake(cell.right, cell.bottom))) {
                bottomRightFlag = YES;
            }
        }
    }
    if (topLeftFlag && topRightFlag && bottomLeftFlag && bottomRightFlag) {
        return;
    }
    ...
    for (WDHorizontalTableViewCell *cell in [self subviews]) {
        if ([cell isKindOfClass:[WDHorizontalTableViewCell class]]) {
            if (!CGRectIntersectsRect(currentRect, cell.frame) && [cell isKindOfClass:[WDHorizontalTableViewCell class]] ) {
